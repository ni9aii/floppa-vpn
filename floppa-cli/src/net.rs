...
/// Whether `interface` currently exists. Used by `stop` to short-circuit when nothing is
/// connected, instead of chasing a process that has already torn its tunnel down.
